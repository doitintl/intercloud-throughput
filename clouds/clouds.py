...
class CloudRegion:
    ...
    def script(self):
        return f"{_root_dir()}/subscripts/{self.lowercase_cloud_name()}-launch.sh"

    def deletion_script(self):
        return f"{_root_dir()}/subscripts/{self.lowercase_cloud_name()}-delete-instances.sh"

    def script_for_test_from_region(self):
        return f"{_root_dir()}/subscripts/do-one-test-from-{self.lowercase_cloud_name()}.sh"
