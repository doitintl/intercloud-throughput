...
SERVER_CREATION_OUTPUT=$(gcloud compute instances create ${SERVER_NAME} \
  ...
  --metadata-from-file=startup-script="$SCRIPT_DIR"/startup-scripts/gcp-install-and-run-iperf-server.sh
  )
...
gcloud compute instances create ${CLIENT_NAME} \
   ...
   --machine-type=e2-micro \
   --network-interface=network-tier=PREMIUM  \
   --metadata-from-file=startup-script="$SCRIPT_DIR"/startup-scripts/gcp-install-iperf.sh
